...
### IO模型
	系统层级的io读写，就是read和write操作，指的是内存缓冲区和进程缓冲区之间的数据交换过程。而将外部数据与内存缓冲区的交换过程，会导致操作系统中断，这属于操作系统的范围。
	1. BIO：同步阻塞IO，当内存缓冲区还没有数据可以同步给进程的时候，这时候会进行等待挂起，直到数据同步完成
		优点：操作简单、等待会释放cpu资源
		缺点：当有很多连接时，会需要很多线程来维护连接
	2. NIO(NOT BIO)，同步非阻塞IO，当内存缓冲区没有数据可以同步给进程时，先返回空，然后主动轮询获取数据
		优点：不会造成资源等待，可以去处理其它任务
		缺点：主动轮询的话，会造成cpu资源一直占着，导致cpu资源消费。同时当有很多连接的时候，会造成很多线程来维护这些连接
	3. 多路复用(NIO，SELECT模式)：通过selector来注册监听(内存缓冲区同步好进程的时候，程序才去处理这一个连接的数据)、
		优点：只需要一个线程来处理批量连接，适合高并发的场景
		缺点：在select选择也是阻塞等待
...
| 表达式        		| 功能      |	举例	|
| --------     			| -----:   | :----: |
| (?<=xxx)      |   逆向匹配，找到xxx所在表达式，然后逆向开始匹配  |   正则：(?<=windows)2003, windos98,通过matcher是无法匹配的，windows2003，group之后得到的值为2003  |
| (?=xxx)        |正向匹配，找到xxx所在的表达式      |   .*(?=test)，字符串abcdtest，得到的是abcd    |
| (?!xxx)        | 正向非匹配     |       |
| yyy(?:a\|b)	|	获取为yyya或者yyyb	|  mou(?:ce|nt)，mouce和mount都能够匹配，相当于mouce|mount